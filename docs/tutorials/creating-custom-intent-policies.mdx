...
7. Test and debug policy evaluation
...
│   │                                                              │   │
│   │  1. Load the policy assigned to the agent                   │   │
│   │  2. Build the policy input                                  │   │
│   │  3. Sort rules by descending priority                       │   │
│   │  4. Evaluate every condition in each rule                   │   │
│   │  5. Return the first matching rule                          │   │
│   │  6. Use the policy default when no rule matches             │   │
│   └──────────────────────────────┬───────────────────────────────┘   │
│                                  │                                   │
│                                  ▼                                   │
│                    Allow, Deny, or HITL                              │
...
The internal provider retrieves the policy assigned through `RequestContext.PolicyName`. When no policy is assigned, the provider allows the request with the reason `No policy assigned`. If the named policy cannot be found, the request is denied.
